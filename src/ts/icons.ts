...
export function getURLfromSVG(svg: string) {
	return URL.createObjectURL(new Blob([svg], { type: "image/svg+xml" }));
}
export function getCSSfromURL(url: string) {
	return `url('${url}')`;
}
export function getCSSfromSVG(svg: string) {
	return `url('${getURLfromSVG(svg)}')`;
}
...
export function getIconURL(
	...
	return getURLfromSVG(getIcon(name, colour, colour2));
}
export function getIconCSS(
	...
	return getCSSfromSVG(getIcon(name, colour, colour2));
}
