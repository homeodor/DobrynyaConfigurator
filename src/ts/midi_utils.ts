...
export function octaveRangeToInline(v: number): string {
	return String(v - 2);
}
export function octaveInlineToRange(v: string): number {
	return parseInt(v) + 2;
}
...
export function noteMidiToHuman(note: number): string {
	...
	return (
		Notes[noteAndOctave[0]] +
		(noteAndOctave[1] - octaveDAWOffset)
	).replace("-", "–");
}

export function currentKeyInfoToKey(bank: BranchBank): KeyInfo | false {
	return bank?.bank?.keyinfo && bank?.bank?.keyinfo !== -1
		? keyInfoToKeyObject(bank.bank.keyinfo)
		: false;
}
...
export function getNoteInCurrentScale(
	padNo: number,
	bank: BranchBank
): NoteInScaleData {
...
function getNoteInScale(
	...
	let theScale = scales[(key as KeyInfo).mode];

	let sumOffset: number = pad + (key as KeyInfo).offset;

	let offsetFromScale =
		theScale.totalnotes >= 12
	...
	let noteValue =
		(key as KeyInfo).key +
		((key as KeyInfo).octave +
			Math.trunc(sumOffset / theScale.totalnotes)) *
			12 +
		offsetFromScale;
...
export function eightToSeven(arr: any): number[] {
	...
	for (
		let i = 0;
		i < arr.length; // we are not incrementing the variable!
