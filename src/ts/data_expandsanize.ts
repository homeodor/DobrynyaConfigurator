export function expandData(model: any, data: any) {
	for (let property in model) {
		if (typeof model[property] == "object") {
			if (property in data) expandData(model[property], data[property]);
			else data[property] = structuredClone(model[property]);
		} else {
...
export function sanizeData(model: any, data: any, safe: any = false): boolean {
	// removes default values. Assumes that data has all the keys of model
		...
		for (let property in data) {
			if (!(property in model)) {
				if (!safe) {
					...
					console.log(data, model);
					console.warn(
						`Sanize warning: property ${property} is not in model`
					);
				}
				continue;
			}
			...
			if (typeof data[property] == "object") {
				if (sanizeData(model[property], data[property]))
					delete data[property];
			} else {
				if (model[property] == data[property]) delete data[property];
				else hadNonDefaultData = true;
			}
		}
...
export function expandSetSanize(
	model: any,
	data: any,
	set: Function,
	safe = true
...
export interface ExpanderSanizerData<T> {
	model: T;
	data: T;
}

export class ExpanderSanizer<T> {
	static mapID = 0;
	...
	#dataStorage: ExpanderSanizerData<T>;
	#reExpand: boolean = true;
	#theMapID: number;
	#cleanup: Function;

	constructor(v: ExpanderSanizerData<T>, cleanup: Function = null) {
