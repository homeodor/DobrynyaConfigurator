...
interface BootloaderData {
	device: HIDDeviceShim | null;
	deviceID: string;
	flashPageSize: number | null;
	flashNumPages: number | null;
	maxMessageSize: number | null;
	familyID: any;
	dbrID: any;
	blVersion: string | null;
	fwVersion: string | null;
	fwVersionIsReal: boolean | null;
}
...
export async function exitBootloader() {
	if (!bootloader.device) {
		throw new Error("Bootloader shim must be defined by this point");
	}

	await requestAndWait(bootloader.device, hf2Command.RESET_INTO_APP);
}

export async function writeUF2Data(requestArray: Uint8Array) {
	if (!bootloader.device) {
		throw new Error("Bootloader shim must be defined by this point");
	}
...
async function getBasicData() {
	...
	let bootloaderDataArr = await requestAndWait(
		bootloader.device,
		hf2Command.BININFO
	);
...
async function getVersionData() {
	if (bootloader.flashNumPages == null || bootloader.flashPageSize == null)
		return false;
	...
	let bytesizepos =
		bootloader.flashNumPages * bootloader.flashPageSize - 0x24; // 36 bytes
	...
	let result = new TextDecoder().decode(
		await requestAndWait(
			bootloader.device,
			hf2Command.READ_WORDS,
			new Uint8Array(dataStuff)
		)
	);
...
export async function dumpFirmware(ev: MouseEvent) {
	...
	if (
		bootloader.flashNumPages === null ||
		bootloader.flashPageSize === null ||
		bootloader.device === null
	) {
		throw new Error(
			"Bootloader shim, flashNumPages and flashPageSize must be defined by this point"
		);
	}
...
export async function getBootloaderData() {
	...
	let bootloaderDataArr = new TextDecoder()
		.decode(await requestAndWait(bootloader.device, hf2Command.INFO))
		.trim()
		.split("\r\n");

	let bootloaderData: Record<string, string> = {};
	...
	if ("Dbr-version" in bootloaderData) {
		// new bootloader
		bootloader.deviceID = bootloaderData["Board-ID"]
			.split("-")
			.slice(2)
			.join("-");
		...
		if (board == "MiniV2" || board == "MicroV2")
			bootloader.deviceID = `${board.toLowerCase()}-${
				chip.startsWith("SAMD21G17") ? "17" : "18"
			}`;
		else bootloader.deviceID = "32r1";
...
async function requestAndWait(
	dev: HIDDeviceShim,
	cmd: number,
	data: Uint8Array | number[] | null = null
): Promise<Uint8Array> {
	if (data === null) data = [];

	let theArray = new Uint8Array(
		((data as Uint8Array).byteLength ?? data.length) + 8
	);
	...
	let resultTotal: Uint8Array = await new Promise((resolve, reject) => {
		...
		let dataHandler = (ev: HIDEventShim) => {
			...
			if (isFirst) {
				...
				if (receiveTag != tag || ev.data.getUint8(3)) reject();
			}
			...
			result.set(
				new Uint8Array(ev.data.buffer).slice(offset, length + 1),
				byteLength
			);
			...
			if (isFinal) {
				dev.removeEventListener(
					"inputreport",
					dataHandler as EventListener
				);
				resolve(result);
			}
		};

		dev.addEventListener("inputreport", dataHandler as EventListener);

		for (let arr of sendArrays) dev.sendReport(0, arr);
	});
