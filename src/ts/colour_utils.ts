...
export function getH(hex: HexColour): number {
	return hex >> 8;
}
export function getS(hex: HexColour): number {
	return (hex >> 4) & 0xf;
}
export function getV(hex: HexColour): number {
	return hex & 0xf;
}

export function hexFromComponents(h: number, s: number, v: number): number {
...
export function isSameColour(what: HexColour, against: HexColour): boolean {
	...
	if (
		againstHSV.s == 0 &&
		againstHSV.s == whatHSV.s &&
		againstHSV.v == whatHSV.v
	)
		return true; // shade of gray
...
export function hueShiftPattern(currentPatchPattern: Pattern) {
	...
	currentPatchPattern.forEach((v, k) => {
		if ((v & 0xf) != 0)
			// not black
			currentPatchPattern[k] =
				((((v >> 8) + hueOffset) & 0xff) << 8) | (v & 0xff);
		else currentPatchPattern[k] = 0; // if it is black, just make it zero
	});
}
