...
export function fillWithTemplate(
	bank: BranchBank,
	action: string,
	key: { trigger: boolean; toggle: boolean; key: string } | string,
	basevalue: number = 0,
	increment: number = 1
) {
	let isTrigger = false;
	...
	if (bank.pads === undefined) {
		bank.pads = [];
	}

	for (let i = 0; i < numberOfPads; i++) {
		if (bank.pads[i] == undefined) {
			bank.pads[i] = {};
		}
		...
		if ("midi" in thePad) {
			if (action == "fill") delete thePad.midi;
			else if (action == "append") {
				if (what == "cc") {
					for (let keyCC of removeCCAuxKeys) {
						if (thePad.midi && keyCC in thePad.midi) {
							delete thePad.midi[keyCC];
						}
					}
				}
			}
...
export function getNewPatchName(
	...
	do {
		if (!thePatchName) suggestedPatchName = `New ${i++}`;
		else {
			suggestedPatchName = `${
				thePatchName
		...
		patchesInfo.find(v => {
			return v.name === `${suggestedPatchName}.dbrpatch`;
		})
	);
...
export function checkIfPatchNameIsValid(
	testName: string,
	patchesInfo: PatchInfoItem[]
): NameFailsBecause {
	...
	if (patchesInfo.find((v: PatchInfoItem) => v.name === testName))
		return NameFailsBecause.Exists;
