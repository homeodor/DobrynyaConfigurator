export const keyboardJSToHIDCatchable: Record<string, number> = {
	KeyA: 4,
	KeyB: 5,
	KeyC: 6,
	KeyD: 7,
...
export const keyboardProperNames: Record<number, string> = {
	45: "–",
	46: "=",
	47: "[",
	48: "]",
...
export const keyboardJStoHID: Record<string, number> = {
...
export const keyboardCombinations: Record<string, number | "notacombo"> = {
	Mac: "notacombo",
...
*/
	...
	"Win key": keyboardJSToHIDCatchable["MetaLeft"], //opens the Start Menu
...
export const keyboardShortNames: Record<number, string> = {
	42: "Bksp",
	57: "Caps",
	75: "Pg Up",
	76: "Del",
...
export const keyboardMedia: Record<string, number> = {
	// Power Control
	Power: 0x0030,
	Reset: 0x0031,
	Sleep: 0x0032,
...
export function ctrlCmd(ev: KeyboardEvent) {
...
export function keycodeToHuman(kc: number, returnShort: boolean = false): any {
	...
	if (isMedia) {
		...
		let val: any = properName
			? properName
			: getKeyByValue(keyboardJSToHIDCatchable, kc);
		...
		if (modifiers & 0x8)
			val = (isMacLikeLocal ? "⌘" : returnShort ? "⊞" : "Win+") + val;
		if (modifiers & 0x2)
			val = (isMacLikeLocal || returnShort ? "⇧" : "Shift+") + val;
		if (modifiers & 0x4)
			val = (isMacLikeLocal ? "⌥" : returnShort ? "⎇" : "Alt+") + val;
		if (modifiers & 0x1)
			val = (isMacLikeLocal || returnShort ? "⌃" : "Ctrl+") + val;
