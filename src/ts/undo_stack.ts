export interface StackObjectProxy {
	data: any;
}
...
export default class UndoStack {
	...
	push(): boolean {
		if (
			JSON.stringify(this.#stack[this.#pointer]) ===
			JSON.stringify(this.#theObject.data)
		) {
			console.log(
				"Tried pushing undo, but no change happened...",
				this.#theObject.data
			);
			return false;
		}
		...
		if (this.#pointer < this.#maxUndos)
			this.#stack[++this.#pointer] = structuredClone(
				this.#theObject.data
			);
		else {
			this.#stack.shift();
	...
	#mutate() {
		this.#theObject.data = structuredClone(this.#stack[this.#pointer]);
	}
