...
function sevenToEight(d: Uint8Array, hasFilename: boolean = false): SevenToEightData {
	...
	return { filename: hasFilename ? Uint8Array.from(filename) : null, data: Uint8Array.from(outArray) };
...
function versionDataToString(versionArray: Uint8Array) {
	...
	while (!versionArrayNormal[versionArrayNormal.length - 1]) versionArrayNormal.pop();
