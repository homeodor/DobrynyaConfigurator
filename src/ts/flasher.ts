...
interface UF2ParsedChunk {
	address: number;
	data: Uint8Array;
}
interface UF2StorageItem {
	data: null | UF2ParsedChunk[];
	state: UF2State;
}
interface UF2Storage {
	bootloader: UF2StorageItem;
	firmware: UF2StorageItem;
}
...
export function textState(theState: UF2State) {
	switch (theState) {
		case UF2State.Empty:
			return "";
		case UF2State.Downloading:
			return "Downloading...";
		case UF2State.Failed:
			return "Server not available";
		case UF2State.Downloaded:
			return "Ready for upload";
		case UF2State.Custom:
			return `<i>${customUF2Name}</i>`;
	}
}

export function getFirmwareDataState(): UF2State {
	return uf2Storage.firmware.state;
}
export function getBootloaderDataState(): UF2State {
	return uf2Storage.bootloader.state;
}
...
let uiUpdateFunc: null | Function = null;
let updateProgress: null | Function = null;
let updateMaxProgress: null | Function = null;

export function setUiUpdate(v: Function, p: Function, mp: Function) {
	uiUpdateFunc = v;
	updateProgress = p;
	updateMaxProgress = mp;
}
...
export function customUF2(ev: DragEvent) {
	...
	if (!ev.dataTransfer) {
		throw new Error("DataTransfer of the drop has been null");
	}
...
async function getUF2Data(
	model: string,
	target: UF2StorageItem,
	...
	try {
		response = await fetch(
			`https://dobrynyadev.kt8.ru/firmware/${model}${maybeBootloader}/latest/`,
			{
				mode: "cors",
			}
		);
...
async function burn(target: UF2StorageItem) {
	if (
		!target.data ||
		(target.state != UF2State.Downloaded && target.state != UF2State.Custom)
	)
		return;

	if (!updateMaxProgress || !updateProgress) {
		throw new Error("UI update functions must be defined at this point");
	}
	...
	for (let chunk of target.data) {
		...
		let requestArray = new Uint8Array([
			...new Uint8Array(pageAddress.buffer, 0, 4),
			...chunk.data,
		]);
...
export async function burnBootloader() {
	return burn(uf2Storage.bootloader);
}
export async function burnFirmware() {
	return burn(uf2Storage.firmware);
}
...
function parseUF2(arrayBuffer: ArrayBuffer, target: UF2StorageItem) {
	...
	for (let i = 0; i < arr8.length; i += 512) {
		...
		if (lengthOfData != 256)
			console.warn(`Data length is ${lengthOfData} at offset ${i}`);
