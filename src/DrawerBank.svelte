<script lang="ts">
	...
	let allPadsToScale = () => {
		fillWithTemplate(currentBank, "fill", "scale");
	...
	$: {
		...
		currentBank.bank.keyinfo = keyinfoPrev =
			scaleEnabled && scale.key != -1 ? keyObjectToKeyInfo(scale) : -1;
		...
		if (lightshowPrev != currentBank.bank.lightshow) {
			// external change... probably in the beginning
	...
	<div>
		<fieldset id="dw-bank-colours">
			<legend
				...
				<Halp>
					<p>
						This sets default pad colours. They can be overridden on
						per-pad basis. Any or all colours may be disabled
						altogether.
					</p>
					<p>
					...
					<p>
						...
						The pads that correspond to the root note of the scale will
						have this colour.
					</p>
					<p>
						<i>Root note active colour</i> is like active colour, but
						for pads that correspond to the root note.
					</p>
				</Halp>
			</legend>
		...
		<fieldset id="dw-bank-key" class="blockenablertarget">
				...
				<label>
					...
					<mark
						...
						<Halp
							...
							<p>
								Each pad can still be overridden to send other
								notes or send no note at all; things like CC are
								also available, of course.
							</p>
							<p>
								Encoders can be set to change scale parameters
								on the fly.
							</p>
						</Halp>
					</mark>
			...
			<div class="ce-block">
				<button
					on:click={allPadsToScale}
					disabled={!scaleEnabled || scale.key == -1}
		...
		<fieldset id="dw-bank-midi">
			...
			<div class="ce-block">
				...
				<RangeWithInline
					on:input={patchChanged}
					bind:value={currentBank.bank.vel}
				/>
			</div>
			...
			<div class="ce-block">
				...
				/>
				<p>
					<label class:disabled={channelValue == -1}
						><input
							on:input={patchChanged}
							type="checkbox"
							bind:checked={channelIsGlobal}
							disabled={channelValue == -1}
						/>
						Global
						<Halp>
							With this option, once the bank is selected, all
							controls will send data on this channel, not just
							the pads – unless, of course, those controls have
							their own custom channel setting.
						</Halp></label
					>
				</p>
			</div>
		...
		<fieldset id="dw-bank-lightshow">
			<legend
				...
				<Halp
					><p>
						Lightshows are a rather advanced technique when the host
						(not the Dobrynya itself) drives LEDs of the device for
						various complex effects.
					</p>
					<p>
						If you are interested, google “lightshow launchpad” to
						know more. If you just want some fun colourful effects,
						check out “Bursts” on the patch settings page.
					</p></Halp
				>
			</legend>
			<div class="ce-block">
			...
			<div class="ce-block">
				<label>
					<input
						...
						type="checkbox"
						class="appleswitch"
						bind:checked={lightshowAgnostic}
					/>
					<mark
						>Ignore mapping
						<Halp>
							<p>
								By default, if Dobrynya receives a note, it will
								try to match it against the notes that are
								mapped to the pads. That is, if it receives D4,
								and pad 6 has D4 mapped to it, pad 6 will light
								up. This may be convenient if you use the notes <i
									>sent</i
								> by Dobrynya itself and route them back to Dobrynya
								in your DAW.
							</p>
							<p>
								However, because mappings can be very different
								and may or may not include notes, you may want
								to have consistency instead. This option
								provides exactly that. Incoming C1 will always
								correspond to pad 1 (lower left), C#1 to pad 2,
								and so on all the way to D#2 at pad 16. If you
								are trying out MIDI tracks for lightshows made
								by other users, this is what you want, too.
							</p>
						</Halp>
					</mark>
				</label>
