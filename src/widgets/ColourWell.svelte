<script lang="ts">
	...
	import { gracefulGetColour } from "colour_utils";
	...
	export let coloursArray: ColourArray = [HexColour.off(), HexColour.off()];
	...
	let auxHex = HexColour.off();

	let dispatchEvent = createEventDispatcher();

	let backgroundColourHex = HexColour.off();
	...
	$: {
		if (colourIndex == -1) {
			...
			backgroundColourHex = gracefulGetColour(
				colourIndex,
				coloursArray,
				globalColours,
				isKeyOfScale,
			...
			auxHex = gracefulGetColour(
				colourIndex == 0 ? 1 : 0,
				coloursArray,
				globalColours,
				isKeyOfScale,
				false
			); // the opposite colour, if available
		}
...
<div class="colourwellholder">
	...
	<div
		...
		style="background-color: {backgroundColourHex.toCSS()}"
		on:click={show}
	></div>
	{#if modalIsOpen}
		<Colour
