	...
	<div class="pianorollwhite">
		{#each whiteKeys as theKey, k}
			...
			<div
				...
				class="pianorollkey whitekey {whiteKeyClasses[k]}"
				data-key={theKey}
				title={whiteKeyLetter[k]}
			></div>
		{/each}
	</div>
	<div class="pianorollblack">
		{#each blackKeys as theKey, k}
			...
			<div
				...
				class:thescalekey={musicScaleKey === theKey && musicKey < 0}
				class:thekey={musicKey === theKey}
				class="pianorollkey blackkey {blackKeyClasses[k]}"
				data-key={theKey}
				title={blackKeyLetter[k]}
			></div>
		{/each}
	</div>
</div>
...
<style>
	...
	.whitekey {
		width: calc(0.04 * var(--size-value));
		height: calc(0.13 * var(--size-value));
	...
	.blackkey {
		margin-left: calc(0.019 * var(--size-value));
		width: calc(0.024 * var(--size-value));
		height: calc(0.074 * var(--size-value));
		background-color: rgb(63, 63, 63);
		border-radius: 0 0 0.2em 0.2em;
	}
	.blackkey.csharp {
		margin-left: calc(0.026 * var(--size-value));
	}
	.blackkey.fsharp {
		margin-left: calc(0.052 * var(--size-value));
	}
	.pianorollkey {
		cursor: pointer;
	}
	.pianoroll:not(.disabled) .thekey {
		background: var(--accent-gradient);
		border: none;
	}
	.pianoroll:not(.disabled) .thescalekey {
		background: var(--striped-gradient), var(--white-gradient);
		border: none;
	}
	.octave10 .notonoctave10 {
		visibility: hidden;
	}
</style>
