<script lang="ts">
	...
	export let validatorFunction: Function = (v: any) => {
		v;
		return true;
	};
	...
	$: {
		if (theInline) isValid = validatorFunction(value) || prevValue == value;
		else isValid = true;
	}
</script>
...
<span class="rename-inline-holder" bind:this={inlineHolder}>
	...
	{:else}
		...
		<span
			...
			class:disabled
			class="rename-inline"
			on:click={doNotFocus}>{value}</span
		>
	{/if}
	{#if showDot}
	...
	<svg
		...
		class:disabled
		><g fill="none" stroke-miterlimit="16.0535" stroke-width="1.6054">
			{#if inlineActive}
				<path class="st1" d="M1.5,15l5.6,5.8C7.1,20.8,22,6,22.3,5.7" />
			{:else}
				<path
					d="m22.3 7.7c.3-.3.8-2.3-1.2-4.3s-3.9-1.6-4.3-1.2-15.2 15.2-15.2 15.2c-.1.1-.1.2-.1.4l.6 3.8c.1.3.4.6.7.7l3.9.6c.2 0 .3 0 .4-.1 0 0 14.9-14.8 15.2-15.1z"
				/><path d="m1.6 17.4s1.7-.2 3.7 1.8 1.8 3.7 1.8 3.7" />
			{/if}
		</g></svg
	>
</span>
