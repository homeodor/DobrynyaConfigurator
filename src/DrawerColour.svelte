<script lang="ts">
	...
	let selectableTools = [
		{
			id: Tool.Paintbrush,
			src: "",
			title: "Brush tool: paint your colour",
			alt: "Brush",
			svg: "brush",
		},
		{
			id: Tool.Eyedropper,
			src: "",
			title: "Eyedropper tool: pick your colour",
			alt: "Pick",
			svg: "eyedropper",
		},
		{
			id: Tool.Eraser,
			src: "",
			title: "Eraser tool: clear colour and revert to bank colour",
			alt: "Erase",
			svg: "eraser",
		},
	];
	...
	function keydownHandle(ev: KeyboardEvent) {
		...
		if (ev.key == "b" && !temporaryEraser) setTool(Tool.Paintbrush, true);
		...
		if (ev.key == "i" && !temporaryEraser) setTool(Tool.Eyedropper, true);
	...
	function keyupHandle(ev: KeyboardEvent) {
		...
		if (ev.key == "r")
			if (buttonRandom.disabled) ctDialogs.random.start();
			else buttonRandom.click();
	...
	function setColourPaintModeBankName(): string {
		return colourPaintMode == ColourPaintLayer.Idle
			? isAlt
				? "music key"
				: "idle"
	...
	export function updateDevicePreview(force: boolean = false) {
		if (
			...
			(!mainColourWell || // if nothing is loaded
				...
				Object.values(ctDialogs).find(v => {
					return v.isOpen();
				}) !== undefined) // or any of the advanced tools are open
		) {
			return false;
		}

		let pattrn = assembleLayerFromHexes(
			getCurrentHexes(bank, pattern),
			colourPaintMode
		);
	...
	let ctData: CTData = {
		...
		bank: bank,
		pattern: pattern,
		layer: colourPaintMode,
		hex: hex,
	};

	let ctDialogs: { [index: string]: any } = {
		fill: null,
		random: null,
		fade: null,
		copy: null,
...
{#if bank}
	<div style="display:none">
		I don’t know how to fix this, but Svelte does not update BANK for this
		drawer, so I believe something in the DOM must be dependent on this
		variable?
	</div>
{/if}
...
<div class="drawer" id="dw-colourpaint">
	<GotIt cookieName="colourworks">
		Dobrynya goes into colour preview mode while the “Colour paint” pane is
		open so that you see better what you’re doing. However, this is
		temporary, and you still have to upload the patch to apply the changes.
	</GotIt>
	<p class="explain-caption">
		Click the well to pick a colour, then apply it to the whole bank or
		paint each pad individually! A diagonal dash <nobr
			>(<span
				style="display: inline-block;width: 1.5em;height: 1.5em;vertical-align: middle;background-size: 90%;"
				class="nocolour"
			></span>)</nobr
		>
	...
	<div id="dw-colour-toolbar">
		<div class="colourwellholder" style="margin-top:1em">
			...
			/><br />
			<button
				bind:this={buttonMakeBank}
				disabled={isPattern}
				class="smaller"
				on:click={currentToBankColour}
				style="min-width: 16em"
				>Make bank {colourPaintModeBankName} colour</button
			>
		...
		<div id="dw-colour-stuff-panel">
			<div>
				{#each selectableTools as toolArr}
					<button
						on:click={() => setTool(toolArr.id)}
						class="dw-colour-tools"
						class:sel={tool == toolArr.id}
						title={toolArr.title}
						><img alt={toolArr.alt} src={toolArr.src} /></button
					>
				{/each}
				<span>&nbsp;</span>
				<button
					bind:this={buttonFill}
					on:click={() => ctDialogs.fill.start()}
					class="dw-colour-tools"
					title="Bucket tool: fill everything with this colour"
					><img
						alt="Fill"
						src={getIconURL("bucket", bucketCSS[0], bucketCSS[1])}
					/></button
				>
				<button
					bind:this={buttonRandom}
					class="dw-colour-tools"
					disabled={!isPattern}
					on:click={() => {
					...
					title="Random: randomise pattern"
					><img alt="Random" src={getIconURL("random")} /></button
				>
			</div>
			...
			<div class="dw-colour-stuff">
				<p>
					...
					<span
						...
						on:click={() =>
							(colourPaintMode = ColourPaintLayer.Pattern)}
						class:sel={colourPaintMode == ColourPaintLayer.Pattern}
						class="unreal"
						>Patch pattern
						<Halp
							dark={colourPaintMode == ColourPaintLayer.Pattern}
						>
							Patch pattern is used to distinguish the patch from
							others. Click “Randomise” to get a unique combo!
						</Halp>
					</span>
				</p>
				...
				<p>
					<select class="notselect" bind:value={openCtModal}>
						<option value="notacc">More tools</option>
						<option value="fade">Fade and brighten...</option>
						<option value="copy"
							>Copy layers & complementary colours...</option
						>
