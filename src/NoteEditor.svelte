<script lang="ts">
	...
	$: {
		noteOff =
			scaleNote < 0 ? midiUtils.fakeNoteOff : midiUtils.fakeNoteUseScale;
		...
		if (!theInline || !theInline.isActive()) {
			inlineValue =
				musicKey == -1
					? midiNote == midiUtils.fakeNoteOff
						? "Off"
						: "Scale"
					: midiUtils.noteMidiToHuman(midiNote);
		}
	}
	...
	function calculateNote() {
		...
		if (musicKey == -1) previousMidiNote = midiNote = noteOff;
		else {
			if (previousMidiNote >= midiUtils.fakeNoteUseScale && octave == 0)
				// off or from scale...
				octave = 3; // set octave to a reasonable value

			previousMidiNote = midiNote = midiUtils.noteAndOctaveToMidi(
				musicKey,
				octave
			);
		}
	...
	function updateNoteFromInlineRoutine() {
		...
		if (
			...
			if (inlineValue[1] == "#" || inlineValue[1] == "♯")
				sharpFlatShifter = 1;
			if (inlineValue[1] == "𝄪") sharpFlatShifter = 2; // ??? lol
			if (inlineValue[1] == "b" || inlineValue[1] == "♭")
				sharpFlatShifter = -1;
		...
		inlineValue = inlineValue
			.toUpperCase()
			.replace("–", "-")
			.replace("—", "-"); // replace minus sign / M-dash with a short dash. Sigh. It’s complicated...

		if (inlineValue.charCodeAt(0) < 65 || inlineValue.charCodeAt(0) > 72)
			return; // note letter is wrong
		if (
			inlineValue.length == 2 &&
			isNaN(parseInt(inlineValue.substring(1)))
		)
			return; // octave is wrong

		let nowOctave: number = -1; // this is a deliberately out of range default value

		if (inlineValue.length >= 2)
			nowOctave =
				parseInt(inlineValue.substring(1)) + midiUtils.octaveDAWOffset;
		...
		midiNote = midiUtils.constrainValue(
			midiUtils.noteAndOctaveToMidi(noteIndex, nowOctave) +
				sharpFlatShifter
		);
...
<fieldset
	id="ce-note"
	class="pianorollholder capability-note conditional cond-pad blockenablertarget"
>
	<legend
		>Note: <Inline
			bind:this={theInline}
			bind:value={inlineValue}
			on:change={updateNoteFromInline}
			on:cancel={fixInlineValue}
			display="inline"
		/></legend
	>
	...
	<div class="ce-block">
		...
		<RangeWithInline
			...
			defValue={4}
			elId={"cbv-octave"}
			inlineToRange={midiUtils.octaveInlineToRange}
			rangeToInline={midiUtils.octaveRangeToInline}
			bind:value={octave}
			disabled={midiNote > 0x7f}
		/>
	</div>

	<p
		id="ce-note-usesscaleerror"
		class="hh ce-note-scalerelated"
		class:hh={!(scaleNote == -1 && midiNote == midiUtils.fakeNoteUseScale)}
	>
		Note is supposed to be from the scale, but no <span class="unreal"
			>scale</span
		> is set for this bank
	</p>
	...
	<div class="ce-block">
		...
		<button
			...
			on:click={() => {
				patchChanged();
				midiNote = midiUtils.fakeNoteUseScale;
			}}
			disabled={scaleNote == -1}>Reset to scale</button
		>
	</div>
