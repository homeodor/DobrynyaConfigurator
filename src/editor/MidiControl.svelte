<script lang="ts">
	...
	function ccTemplate(ev: Event) {
		...
		enableMin = false;
		if (upperValue != "l") max = parseInt(upperValue);
	...
	let inlineToRangePitch = function (v: string): number | false {
		let numV = parseInt(v);
	...
	async function forceUpdateRanges() {
		...
		ccRangeMin.updateInline();
		ccRangeMax.updateInline();
	}
	...
	export function setDefaultMinMax() {
		...
		if (
			encmode == EncoderBehaviour.RelativeSigned ||
			encmode == EncoderBehaviour.Relative64Zero ||
			encmode == EncoderBehaviour.Relative2Comp
		) {
			min = 65;
			max = 1;
		} else if (what == MidiCtrl.PITCH) {
			min = 64;
			max = 127;
			console.log("YUP");
		} else {
	...
	export function init() {
		...
		const isAccel =
			theControl.control == Control.AccelX ||
			theControl.control == Control.AccelY;

		isToggle = (theControl.discrete || isAccel) && (par & 0x1) == 0x1;
		isBipolar = isAccel && (par & 0x2) == 0x2;
	}
	...
	function setParameters(value: boolean, position: number) {
		par = value ? par | (1 << position) : par & ~(1 << position);
	}
	...
	$: {
		...
		if (!locked) {
			...
			if (what != prevWhat) {
				...
				if (!canHaveMinMax(what)) {
					// it’s off or discrete, so min&max should be plain off
					min = paramOffNegative;
					max = paramOff;
					enableMin = false;
					enableMax = false;
				} else if (canHaveMinMax(what) && !canHaveMinMax(prevWhat)) {
					// pitch bend or cc, switched from a discrete mode
					setDefaultMinMax();
					enableMin = true;
					enableMax = true;
				}
			...
			if (encoderIsRelative) {
				if (relativeEncoderMinPrev != relativeEncoderMin) {
					...
					relativeEncoderMin = relativeEncoderMinPrev =
						convertAbsoluteToRelative(min);
				...
				if (relativeEncoderMaxPrev != relativeEncoderMax) {
					...
					relativeEncoderMax = relativeEncoderMaxPrev =
						convertAbsoluteToRelative(max);
			}

			if (enableMin && min == paramOffNegative) min = 0;
			else if (!enableMin && min != paramOffNegative)
				min = paramOffNegative;
			if (enableMax && max == paramOff) max = 127;
			else if (!enableMax && max != paramOff) max = paramOff;

			setParameters(isToggle, 0);
			setParameters(isBipolar, 1);
...
<fieldset id="ce-control">
	...
	{#if theControl.control == Control.AccelX || theControl.control == Control.AccelY}
		<div class="ce-block">
			<h4>
				Flat position equals <Halp>
					<p>Tilting can be done in two separate ways.</p>
					<p>
						<i>Middle</i> means that the whole axis will be mapped from
						minimum to maximum, hence the flat position will be in the
						middle between minimum and maximum.
					</p>
					<p>
						<i>Minimum</i> means that the flat position will be the minimum,
						and tilting both ways along the axis will change the value
						towards the maximum.
					</p>
				</Halp>
			</h4>
			<select on:input={patchChanged} bind:value={isBipolar}>
				<option value={false}>Middle</option>
	...
	{#if canHaveMinMaxValue}
		...
		{#if encoderIsRelative && what == MidiCtrl.CC}
			<h4>
				Step <Halp
					>If you turn the encoder slowly in relative mode, it will
					always be sending +1 or –1. But you can set the step to be
					larger when you turn the encoder fast, i.e. +10 or –10 for
					the fastest rotation. Note however that not all host apps
					support steps other than 1.
				</Halp>
			</h4>
			<div class="ce-block ce-dont-remove-margin">
				<h4>Largest step back</h4>
				<RangeWithInline
					bind:this={ccRangeMin}
					on:input={patchChanged}
					width="3.6em"
					min={-64}
					max={63}
					bind:value={relativeEncoderMin}
				/>
			</div>
			<div class="ce-block">
				<h4>Largest step forward</h4>
				<RangeWithInline
					bind:this={ccRangeMax}
					on:input={patchChanged}
					width="3.6em"
					min={-64}
					max={63}
					bind:value={relativeEncoderMax}
				/>
			</div>

			<p class="explain" style="">
				Some applications do not support relative values other than 1
				and –1.
			</p>
		{:else}
			<div
				id="ce-minmax-switcher"
				class:swap-min-max={theControl.discrete}
			>
				<div class="ce-block ce-dont-remove-margin">
						...
						<h4>
							<label>
								<input
									on:input={patchChanged}
									type="checkbox"
									bind:checked={enableMin}
									title="Enable sending value"
								/>
								{#if isToggle}Toggle off{:else}Release{/if}
							</label>
						</h4>
					{/if}
					<RangeWithInline
						...
						on:input={patchChanged}
						width="3.6em"
						inlineToRange={inlineToRangePitch}
						rangeToInline={rangeToInlinePitch}
						bind:value={min}
						disabled={!enableMin}
					/>
				</div>
						...
						<h4>
							<label>
								<input
									on:input={patchChanged}
									type="checkbox"
									bind:checked={enableMax}
									title="Enable sending value"
								/>
								{#if isToggle}Toggle on{:else}Press{/if}
							</label>
						</h4>
					{/if}
					<RangeWithInline
			...
			<div class="checkboxholder">
				...
				<label on:click|stopPropagation|preventDefault={openRampEditor}>
					...
					/> <mark></mark><span class="unreal"
						>Ramp...
						<Halp>
							Ramp allows pitch bend or CC to change smoothly,
							similar to attack and release envelopes. Ramps use
							external clock to operate, so don’t forget to send
							clock from your host to Dobrynya!
						</Halp>
					</span></label
				>
				{#if rampIsOpen}
