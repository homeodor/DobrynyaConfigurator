<script lang="ts">
	...
	let {
		...
		encoderIsTempo: boolean;
		encoderIsScaleOrTempo: boolean;
		cc: number;
		min: number;
		max: number;
		par: number;
...
<div class="ce-block">
	<h4>
		<label>
			<input
				oninput={sendThisAsCC}
				disabled={!encoderIsScaleOrTempo}
				type="checkbox"
				checked={isCC}
	...
	<RangeWithInline
		disabled={!encoderIsScaleOrTempo || !isCC}
		on:input={patchChanged}
		bind:value={cc}
	/>
	{#if encoderIsTempo}
		<p class="warn explain" style="padding:0">
			Tempo will be sent from 60 to 187<wbr /> as CC values 0 to 127.
		</p>
	{/if}
</div>
