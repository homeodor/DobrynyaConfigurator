<script lang="ts">
	...
	$: {
		...
		if (data?.encmode) {
			isRelative =
				data.encmode >= EncoderBehaviour.Relative64Zero &&
				data.encmode <= EncoderBehaviour.RelativeSigned;
			isAuxModeEncoder =
...
<div class="dobrynya-ctrl-internal">
	{#if isAuxModeEncoder}
		<div class="dobrynya-ctrl-encmode">
			{encBehaviourHuman[data.encmode - 4]}
		</div>
	{/if}
	...
	{#if data.midi}
		{#if noteName}
			<div
				class="dobrynya-ctrl-note"
				class:dobrynya-ctrl-note-scale={noteIsFromScale}
			>
				{noteName}
			</div>
		{/if}
		...
		{#if data.midi.cc}
			<div
				class:dobrynya-ctrl-ccfake={!isRealCC}
				class:dobrynya-ctrl-cc={isRealCC && !isRelative}
			>
