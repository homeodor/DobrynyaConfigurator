<script lang="ts">
	...
	$: {
		...
		let noteCode = value
			? rampData.find(v => v.value === lengthValue)!.noteCode.split("-")
			: "";

		if (theSVG) {
			Array.from(theSVG.children).forEach(
				(v: any) => (v.style.display = "none")
			);

			for (let symbol of noteCode) {
				// theSVG.querySelectorAll(`.supernote-${noteCodePart}`)?.forEach();

				if (!isNaN(parseInt(symbol)) && symbol != "1") {
					// numeric...
					let symbolNumeric = parseInt(symbol);

					while (symbolNumeric >= 2) {
						theSVG
							.querySelectorAll(`.supernote-N${symbolNumeric}`)
							.forEach(showEach);
						symbolNumeric /= 2;
					}
					...
					theSVG
						.querySelectorAll(`.supernote-${symbol}`)
						.forEach(showEach);
				}
				...
				if (symbol[0] == "U") {
					...
					if (multi > 1) {
						theSVG
							.querySelectorAll(".supernote-XX")
							.forEach(showEach);
						theSVG
							.querySelectorAll(`.supernote-X${multi}`)
							.forEach(showEach);
					}
				}
...
<fieldset id="modal-{rampID}p">
			...
			<p>
				<label class:disabled={!enabled}
					><input
						...
						disabled={!enabled}
						type="radio"
						bind:group={tickSource}
						value={0x2000}
					/> External (from the host)</label
				>
			</p>
	...
	<div class="supernote-holder">
		...
		<svg
			...
			x="0px"
			y="0px"
			viewBox="0 0 27.61 29.78"
			bind:this={theSVG}
		>
			<path
				class="supernote-N64"
				d="M6.56,5.48V4.82v-0.4c1.3,1.37,2.99,3.29,2.99,5.16c0,0.56-0.09,0.8-0.25,1.28
			...
			<path
				class="supernote-N32"
				d="M6.56,10.03V9.37v-0.4c1.3,1.37,2.99,3.29,2.99,5.16c0,0.56-0.09,0.8-0.25,1.28
					c-0.16,0.47,0.39,1.23,0.39,1.23l0.45-0.9c0.24-0.62,0.27-0.84,0.27-1.61c0-3.51-3.85-5.88-3.85-9.38c0-0.11-0.09-0.2-0.2-0.2H6.11
					c-0.11,0-0.2,0.09-0.2,0.2v5.27L6.56,10.03z"
			/>
			<path
				class="supernote-N8"
				d="M6.56,14.58v-0.66v-0.4c1.3,1.37,2.99,3.29,2.99,5.16c0,0.56-0.09,0.8-0.25,1.28
					c-0.16,0.47,0.39,1.23,0.39,1.23l0.45-0.9c0.24-0.62,0.27-0.84,0.27-1.61c0-3.51-3.85-5.88-3.85-9.38c0-0.11-0.09-0.2-0.2-0.2H6.11
					c-0.11,0-0.2,0.09-0.2,0.2v5.27L6.56,14.58z"
			/>
			<path
				class="supernote-N16"
				d="M6.56,19.14v-0.66v-0.4c1.3,1.37,2.99,3.29,2.99,5.16c0,0.56-0.09,0.8-0.25,1.28
					c-0.16,0.47,0.39,1.23,0.39,1.23l0.45-0.9c0.24-0.62,0.27-0.84,0.27-1.61c0-3.51-3.85-5.88-3.85-9.38c0-0.11-0.09-0.2-0.2-0.2H6.11
					c-0.11,0-0.2,0.09-0.2,0.2v5.27L6.56,19.14z"
			/>
			<path
				class="supernote-N2"
				d="M6.56,9.3c0-0.11-0.09-0.2-0.2-0.2H6.11c-0.11,0-0.2,0.09-0.2,0.2v13.15
			...
			<path
				class="supernote-N4"
				d="M5.74,23.53c0.38,0.5,0.7,1.87-1.64,3.2s-2.92,0.69-3.29,0.14
					c-0.36-0.55-0.58-1.59,1.55-2.78S5.24,22.85,5.74,23.53z"
			/>
			<path
				class="supernote-D"
				d="M10.36,25.05c0-0.62,0.5-1.12,1.12-1.12s1.12,0.5,1.12,1.12
					s-0.5,1.12-1.12,1.12C10.85,26.17,10.36,25.67,10.36,25.05z"
			/>
			<path
				class="supernote-F"
				d="M14.75,16.2c0-0.62,0.5-1.12,1.12-1.12s1.12,0.5,1.12,1.12
					s-0.5,1.12-1.12,1.12S14.75,16.82,14.75,16.2z"
			/>
			<path
				class="supernote-N1"
				d="M6.28,14.22c-1,0-1.37,0.86-1.37,1.75c0,1.53,1.14,2.71,2.67,2.71
			...
			<path
				class="supernote-Q"
				d="M24.48,0.25h-0.25c-0.11,0-0.2,0.09-0.2,0.2V13.6c0,0.26-0.26,0.41-0.49,0.32
			...
			<path
				class="supernote-T"
				d="M4.7,15.14l-1.48,1.44c0.36,0.06,0.64,0.21,0.83,0.43s0.3,0.49,0.3,0.82c0,0.33-0.09,0.62-0.27,0.87
			...
			<g class="supernote-L">
				<path
					d="M8.71,18.67c0.75,0.52,1.67,0.8,2.56,1c0.91,0.18,1.84,0.27,2.78,0.26c0.94,0.01,1.87-0.08,2.78-0.25
			...
			<path
				class="supernote-X2"
				d="M23.93,18.07h2.52l-0.18,0.86h-4.03l0.13-0.68l2.58-2.04c0.33-0.27,0.56-0.49,0.69-0.68
			...
			<path
				class="supernote-X4"
				d="M27.44,17.68h-0.97l-0.25,1.24h-0.99l0.26-1.24h-3l0.13-0.7l3.36-3.53h1.13l-3.17,3.37h1.75
					l0.23-1.1h0.95l-0.22,1.1h0.97L27.44,17.68z"
			/>
			<path
				class="supernote-XX"
				d="M20.59,16.21l0.86,1l-0.59,0.55L20,16.75l-1.13,1.01l-0.49-0.59l1.12-1l-0.86-1l0.59-0.55
					l0.87,1.01l1.13-1.01l0.49,0.59L20.59,16.21z"
			/>
			<path
				class="supernote-X8"
				d="M26.89,16.6c0.11,0.19,0.17,0.4,0.17,0.64c0,0.36-0.1,0.67-0.29,0.93
