...
<script lang="ts">
	...
	function copyLayers(params: CTCopyParams, currentHex: HexArrays) {
		...
		if (
			params.includeBank &&
			!toPattern &&
			(!params.keepexisting || currentHex.bank[params.to] == colourOff)
		)
		...
		if (params.includeExplicit && params.from < 2 && params.to < 2) {
			...
			for (let i in arrFrom) {
				...
				if (srcColour == colourOff && toPattern) {
					if (
						params.includeBank &&
						currentHex.bank[params.from] != colourOff
					)
			...
			if (toPattern) currentHex.pattern = [...arrTo];
			else currentHex.pads[params.to] = [...arrTo];
		}
...
<dialog class="dw-colour-toolmodal modal" bind:this={dialog}>
	...
	<OkCancel
		...
		theDialog={dialog}
		onok={finish}
		oncancel={() => {ctExit(dialog, ctData); oncancel();}}
	/>
</dialog>
