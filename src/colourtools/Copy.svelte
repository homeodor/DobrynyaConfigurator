...
<script lang="ts">
	...
	function finish() {
		ctFinish(dialog, copyLayers, params, ctData);
		dispatchEvent("input");
	}
	...
	$: {
		...
		if (layerPrev != ctData.layer) {
			params.from =
				ctData.layer == ColourPaintLayer.Pattern ? 0 : ctData.layer; // change source layer accordingly if changed in the main UI
			params.to =
				ctData.layer == ColourPaintLayer.Pattern // change target layer to pattern if main UI’s layer is pattern
					? ColourPaintLayer.Pattern
					: params.from == ColourPaintLayer.Idle
						? ColourPaintLayer.Active
						: ColourPaintLayer.Idle; // otherwise change to the opposite of the source
		...
		okEnabled =
			params.includeExplicit ||
			(ctData.layer != ColourPaintLayer.Pattern && params.includeBank); // check if there is actual stuff to copy

		if (ctData.hexStorage) {
			let tempHex = copyLayers(
				params,
	...
	function copyLayers(params: CTCopyParams, currentHex: HexArrays) {
		const toPattern = params.to == ColourPaintLayer.Pattern;

		if (
			params.to == ColourPaintLayer.Off ||
			params.from == ColourPaintLayer.Off
		) {
			throw new Error("copyLayers called with ColourPaintLayer.Off");
		}

		currentHex.before = assembleLayerFromHexes(currentHex, params.from);

		let doAllTransforms = function (
			col: HexColour,
			toPattern: boolean = false
		): HexColour {
			if (col.isOff()) return toPattern ? HexColour.black() : col;

			if (params.inverthue) col = col.invH();
			if (params.saturation == ParamSatVal.Dim) col = col.dimS();
			if (params.saturation == ParamSatVal.Max) col = col.maxS();
			if (params.brightness == ParamSatVal.Dim) col = col.dimV();
			if (params.brightness == ParamSatVal.Max) col = col.maxV();
		...
		if (
			params.includeBank &&
			!toPattern &&
			(!params.keepexisting || currentHex.bank[params.to].isOff())
		)
			currentHex.bank[params.to] = doAllTransforms(
				currentHex.bank[params.from]
			);

		if (params.includeExplicit && params.from < 2 && params.to < 2) {
			let arrFrom = getLayerFromHexes(currentHex, params.from);
			const arrTo = getLayerFromHexes(currentHex, params.to);

			for (let i in arrFrom) {
				if (
					params.keepexisting &&
					((!toPattern && arrTo[i].isOn()) ||
						(toPattern && arrTo[i].isBlack()))
				)
					continue; // keep existing colours is ticked

				let srcColour = arrFrom[i];

				if (srcColour.isOff() && toPattern) {
					if (
						params.includeBank &&
						currentHex.bank[params.from].isOn()
					)
						srcColour = currentHex.bank[params.from];
					else {
						srcColour = HexColour.black();
					}
				}
			...
			// we could use toPattern, but this TS happier
			if (params.to == ColourPaintLayer.Pattern) {
				currentHex.pattern = [...arrTo];
			} else {
				currentHex.pads[params.to] = [...arrTo];
			}
		}
...
<dialog class="dw-colour-toolmodal modal" bind:this={dialog}>
	...
	<p>
		You can copy one layer to another, while making some changes to the
		colours.
	</p>
	...
	<fieldset>
		...
		<div class="checkboxblock">
			<label>
				<input
					type="radio"
					bind:group={params.from}
					value={ColourPaintLayer.Idle}
				/>
				Idle colour
			</label><br />
			<label>
		...
		<div class="checkboxblock">
			...
			<label>
				<input
					type="radio"
					bind:group={params.to}
					value={ColourPaintLayer.Pattern}
				/>
				Patch pattern
			</label>
		</div>
	...
	<fieldset>
		...
		<div class="checkboxblock">
			...
			<label>
				<input
					disabled={ctData.layer == ColourPaintLayer.Pattern}
					type="checkbox"
					bind:checked={params.includeBank}
				/>
				Include bank colour
			</label><br />
			<label>
	...
	<fieldset>
		...
		<div class="checkboxblock">
			<label>
				<input
					type="radio"
					bind:group={params.saturation}
					value={ParamSatVal.Keep}
				/>
				Do not change</label
			>
			<br />
