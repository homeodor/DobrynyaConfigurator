...
<script lang="ts">
	...
	export function start() {
		...
		params.layersIdle = ctData.layer == ColourPaintLayer.Idle;
		params.layersActive = ctData.layer == ColourPaintLayer.Active;
		params.layersPattern = ctData.layer == ColourPaintLayer.Pattern;
		reseed();
	}
	function finish() {
	...
	let params: CTRandomParams = {
		layersIdle: ctData.layer == ColourPaintLayer.Idle,
		layersActive: ctData.layer == ColourPaintLayer.Active,
		layersPattern: ctData.layer == ColourPaintLayer.Pattern,
		keepColours: true,
		hueMax: 255,
		hueMin: 0,
		satMax: 15,
	...
	$: {
		okEnabled =
			params.layersIdle || params.layersActive || params.layersPattern;
	...
	function randomFill(
		...
		if (params.layersActive) {
			affectLayers.push(ColourPaintLayer.Active);
		}

		if (params.layersPattern) {
		...
		let vMin: number =
			params.valMin < params.valMax ? params.valMin : params.valMax;
		let vMax: number =
			params.valMin < params.valMax ? params.valMax : params.valMin;
		...
		preview.idle = assembleLayerFromHexes(
			currentHex,
			ColourPaintLayer.Idle
		);
		preview.active = assembleLayerFromHexes(
			currentHex,
			ColourPaintLayer.Active
		);
		preview.pattern = assembleLayerFromHexes(
			currentHex,
...
<dialog bind:this={dialog} class="dw-colour-toolmodal modal">
	...
	<fieldset>
		<button class="dw-colour-tools" on:click={reseed} title="Reseed"
			><img
				alt="Reseed"
				src={getIconURL("random")}
				style="vertical-align: -0.29em;"
	...
	<fieldset>
		<div id="ct-preview" class="preview-with-layers">
			<div>
				<label
					><input type="checkbox" bind:checked={params.layersIdle} /> Idle</label
				><br />
				<PreviewSingle
					hexArray={preview.idle}
					inline={true}
					targetPreview={enableTargetPreview && params.layersIdle}
				/>
			</div>
			<div>
				<label
					><input
						type="checkbox"
						bind:checked={params.layersActive}
					/> Active</label
				><br />
				<PreviewSingle
					hexArray={preview.active}
					inline={true}
			...
			<div>
				<label
					><input
						type="checkbox"
						bind:checked={params.layersPattern}
					/> Pattern</label
				><br />
				<PreviewSingle
					hexArray={preview.pattern}
					inline={true}
					targetPreview={enableTargetPreview &&
						!params.layersIdle &&
						!params.layersActive &&
						params.layersPattern}
				/>
			</div>
		</div>
		<legend>Fill layers</legend>
		<div class="checkboxblock"></div>
		<br /><br />
		<label>
		...
		<div class="doublepoleholder">
			...
			<section class="range-doublepole range-doublepole-hue">
				<input
					min="0"
					max="255"
					bind:value={params.hueMin}
					type="range"
				/>
				<input
					class="colour-slider-h"
					min="0"
					max="255"
					bind:value={params.hueMax}
					type="range"
				/>
			</section>
		</div>
		<div class="doublepoleholder">
			...
			<section class="range-doublepole range-doublepole-sat">
				<input
					min="0"
					max="15"
					bind:value={params.satMin}
					type="range"
				/>
				<input
					class="colour-slider-s"
					min="0"
					max="15"
					bind:value={params.satMax}
					type="range"
				/>
			</section>
		</div>
		<div class="doublepoleholder">
			...
			<section class="range-doublepole range-doublepole-val">
				<input
					min="0"
					max="15"
					bind:value={params.valMin}
					type="range"
				/>
				<input
					class="colour-slider-v"
					min="0"
					max="15"
					bind:value={params.valMax}
					type="range"
				/>
			</section>
		</div>
	...
	<fieldset>
		...
		<label>
			...
			<Halp
				>If selected, each pad will have the same hue across all layers
				selected in the “fill” group. This is useful if for instance you
				want “idle” and “active” layers to have the same hues, but
				different brightness, so that pads just light up brighter when
				hit.
			</Halp>
		</label>
	</fieldset>
	<fieldset>
		<legend
			...
			<Halp>
				<p>
					Oftentimes you want to randomise hues, but keep saturation
					(and probably brightness) consistent. This setting will help
					you do exactly this.
				</p>
				<p>
					<i>Just use maximum</i> will ignore the lower margin of the range
					altogether. All hues will go as saturated as you allow them to
					be.
				</p>
				<p>
					<i>Use max for active, min for idle</i> will set minimum range
					value as the one for “idle” layer, and the maximum for “active”.
					This is useful if you want your randomised hues to “pop” more
					on hitting the pad.
				</p>
				<p>
					<i>Random</i> will pick random saturation values within the range.
				</p>
			</Halp>
		</legend>
		<div class="checkboxblock">
			<label>
				<input
					type="radio"
					bind:group={params.satMode}
					value={ParamSatVal.Max}
				/>
				Just use maximum</label
			><br />
			<label>
				<input
					type="radio"
					bind:group={params.satMode}
					value={ParamSatVal.Reasonable}
				/>
				Max for active, min for idle</label
			><br />
			<label>
				<input
					type="radio"
					bind:group={params.satMode}
					value={ParamSatVal.Random}
				/>
				Random</label
			>
		</div>
	...
	<fieldset>
		<legend
			...
			<Halp>
				<p>
					Oftentimes you want to randomise hues, but keep brightness
					(and probably saturation) consistent. This setting will help
					you do exactly this.
				</p>
				<p>
					<i>Just use maximum</i> will ignore the lower margin of the range
					altogether. All hues will go as bright as you allow them to be.
				</p>
				<p>
					<i>Use max for active, min for idle</i> will set minimum range
					value as the one for “idle” layer, and the maximum for “active”.
				...
				<p>
					<i>Random</i> will pick random brightness values within the range.
				</p>
			</Halp>
		</legend>
		<div class="checkboxblock">
...
<style>
	...
	.doublepoleholder section {
		margin-top: 1em;
		height: 1em;
	}

	.preview-with-layers > div {
		width: 6em;
	}
	.preview-with-layers label {
		margin-bottom: 0.8em;
