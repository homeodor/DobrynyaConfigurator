...
export function fill(
	...
	if (mode == FillMode.Solid) {
		...
		let colourSafe1 =
			colour1 == colourOff
				? currentHex.bank[currentLayer] == colourOff
					? 0
	...
	for (let i in currentLayerArray) {
		if (
			(params.affect == CTAffect.Bank &&
				colourIsExplicitlySet(currentLayerArray[i], currentLayer)) ||
			(params.affect == CTAffect.Explicit &&
				!colourIsExplicitlySet(currentLayerArray[i], currentLayer))
		)
			continue;

		if (currentLayer == -1) currentHex.pattern[i] = fillarray[i];
		else currentHex.pads[currentLayer][i] = fillarray[i];
	}
