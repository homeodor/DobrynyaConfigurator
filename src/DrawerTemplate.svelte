<script lang="ts">
	...
	const titleAppend =
		"Adds to whatever is present in the bank, replacing conficting settings";
	...
	function ccTemplate(mode: string) {
		fillWithTemplate(
			currentBank,
			mode,
			{ key: "cc", toggle: ccHow == "t", trigger: ccHow == "g" },
			ccStart
		);
		currentBank = currentBank;
		patchChanged();
	...
	function scaleToNotes() {
		...
		for (let i = 0; i < numberOfPads; i++) {
			...
			if (
				!("midi" in thePad) ||
				!("note" in thePad.midi) ||
				thePad.midi.note != fakeNoteUseScale
			)
				continue;
	...
	$: {
		scaleIsSet =
			currentBank?.bank?.keyinfo != undefined &&
...
<div class="drawer columnizer-in" id="dw-banktemplates">
	<p class="explain-caption">
		Set things off by filling your bank with a preset template!
	</p>
	<div>
		<fieldset id="dw-bankt-reset">
			<legend
				...
				<Halp
					><p>
						Fills the bank with notes typical for a pad group in a
						drum rack in Ableton Live and other DAWs.
					</p>
					<p>
						Typically, Dobrynya has groups 1–4 on banks 1–4, groups
						–3 and –1 on sub-banks 1 and 2, and group 5 on sub-bank
						3.
					</p>
				</Halp>
			</legend>
			...
			<div class="ce-block">
				<button
					on:click={() => drumRackTemplate("fill")}
					title={titleFill}>Clear and fill</button
				>
				<button
					on:click={() => drumRackTemplate("append")}
					title={titleAppend}>Add</button
				>
			</div>
		</fieldset>
		...
		<fieldset id="dw-bankt-reset">
			<legend
				...
				<Halp
					><p>
						Fills the bank with progressive CCs, starting with the
						set number. All values will be set as 127 for maximum
						and 0 for minimum (the whole range).
					</p>
					<p>
						<i>Momentary</i> sends maximum on press and minimum on release.
					</p>
					<p>
						<i>Toggle</i> sends maximum on the first press and minimum
						on the next press.
					</p>
					<p>
						<i>Trigger</i> is the same as momentary but sends maximum
						only.
					</p>
					<p>
						<i>Clear and fill</i> will clear everything that is
						present in current bank pads and fill the CCs, while
						<i>Add</i> will keep everything but the CCs (if any were
						set).
					</p></Halp
				>
			</legend>
			...
			<div class="ce-block">
				<button on:click={() => ccTemplate("fill")} title={titleFill}
					>Clear and fill</button
				>
				<button
					on:click={() => ccTemplate("append")}
					title={titleAppend}>Add</button
				>
			</div>
		</fieldset>
		...
		<fieldset>
			...
			<p>
				You can set scale in <span
					role="link"
					tabindex="0"
					class="unreal"
					on:click={() => drawer("banksettings")}>bank settings</span
				>.
			</p>
		</fieldset>
		...
		<fieldset id="dw-bankt-reset">
			...
			<div class="ce-block">
				<button disabled={!scaleIsSet} on:click={scaleToNotes}
					>Implicit scale to explicit notes
					<Halp
						>All pads that obey the scale will have that note set
						explicitly.</Halp
					>
				</button>
			</div>
			...
			<div class="ce-block">
				<button
					class="dangerous"
					on:click={purgeTheBank}
					disabled={numberOfActiveBanks <= 1}
					>Purge the bank
					<Halp
						>Clears all settings for the bank, including colours and
						pads. The device won’t open empty banks.</Halp
					>
				</button>
			</div>
			...
			{#if numberOfActiveBanks < 2}
				<p class="explain">
					The device won’t open purged banks, so it’s best to have at
					least one bank active in the patch (even if it doesn’t do
					anything).
				</p>
			{/if}
			...
			<Confirm bind:this={purgeConfirm} okText="Purge">
				<p>
					This will purge the bank, erasing all settings, including
					bank parameters such as colour and scale, as well as all
					pads’ parameters.
				</p>
				<p>
					The device ignores purged banks, so this bank will
					essentially be off.
				</p>
			</Confirm>
		</fieldset>
